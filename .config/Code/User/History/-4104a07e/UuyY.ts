...
export class CashierRepository {
  ...
  async getCashes(employee_id: string, parking_id: string, page: number, page_size: number) {
    const { data } = await this.api.get(`cashier/cashiers/?employee=${employee_id}&parking=${parking_id}&page=${page}&page_size=${page_size}`)
    return data
  }
  ...
  async getCashesByStatus(status: string, page: number, page_size: number, parkingId: string) {
    const { data } = await this.api.get(`cashier/cashiers/?status=${status}&page=${page}&page_size=${page_size}&parking=${parkingId}`)
    return data
  }
  ...
  async openCash(parkingId: string, employeeId: string) {

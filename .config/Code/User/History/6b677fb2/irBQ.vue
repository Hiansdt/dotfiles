...
const viewCash = (id: string) => {
  router.push({ name: "cash-details", params: { id } })
}
...
<template>
  <GenericTable key-word="Caixa" v-model:headers="cashController.myCashesHeaders" v-model:search="cashController.search"
    ...
    <VDataTable v-if="cashController.myCashes.length"
      ...
      :loading="cashController.loading" :items-per-page="cashController.myCashesPagination.pageSize"
  ...
  <VDialog v-model="showCreateModal" max-width="500">
    <VCard class="pa-4">
      ...
      <VCardActions>
        <VRow>
          <VCol cols="6">
            <VBtn block variant="outlined" color="" @click="showCreateModal = false">
              Cancelar
            </VBtn>
          </VCol>
          <VCol cols="6">

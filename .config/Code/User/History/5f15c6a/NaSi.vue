...
const paymentMethods = computed(() => {
  return new Set(
    sessionPaymentController.sessionPayments.map(
      (sessionPayment) => sessionPayment.payment_method,
    ),
  );
});
    ...
    <VCol cols="12" class="pa-0">
      ...
      >
        <VWindowItem>
          <VCard class="pa-4">
            <VRow>
              <VCol v-for="paymentMethod in paymentMethods">
                <VCard class="pa-2" variant="outlined">
                  <VCardTitle>
                    {{ paymentMethod }}
                  </VCardTitle>
                  <VCardSubtitle> R$ {{ formatBrl(calculatePaymentMethodTotal(paymentMethod)) }} </VCardSubtitle>
                </VCard>
              </VCol>
            </VRow>
          </VCard>

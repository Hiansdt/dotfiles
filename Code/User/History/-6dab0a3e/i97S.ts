...
import {
  ICashController,
  ICashState,
} from "../interfaces/check/cashController";
import { container } from "tsyringe";
import { CashierRepository } from "../repository/cashierRepository";
...
import { IExtractService } from "../entities/cashier";
...
export const useCashController = defineStore<"cash", ICashController>(
  ...
  () => {
    ...
    const cashierRepository = container.resolve(CashierRepository);
    ...
    const headers = computed({
      get: () => {
        const paymentMethods = [...new Set(state.cashes.map((cashier) => cashier.payment_methods).map((item) => item.map((payment) => payment.method)).flat())];
    ...
    const getCashesByStatus = async (status: string) => {
      ...
      const data = await cashierRepository.getCashesByStatus(
        status,
    ...
    const getMyCashes = async () => {
      ...
      const parking_id = authController.currentParking?.id
      const data = await cashierRepository.getCashes(employee_id, parking_id, state.myCashesPagination.page, state.myCashesPagination.page_size)
    ...
    const getCashById = async (id: string) => {
      state.loading = true
      const data = await cashierRepository.getCashById(id)
      state.loading = false
      return data
    }
    ...
    const closeCash = async (cashId: string) => {
      state.loading = true
      await cashierRepository.closeCash(cashId)
      state.loading = false
    }
    ...
    const approveCash = async (cashId: string) => {
      ...
      await cashierRepository.approveCash(cashId, employeeId)
      state.loading = false
    }
    ...
    const rejectCash = async (cashId: string) => {
      state.loading = true
      const employeeId = authController.currentEmployeeId
      await cashierRepository.rejectCash(cashId, employeeId)
      state.loading = false
    }

    const recheckCash = async (cashId: string) => {
      state.loading = true
      await cashierRepository.recheckCash(cashId)
      state.loading = false
    }

    const concludeCash = async (cashId: string) => {
      state.loading = true
      await cashierRepository.concludeCash(cashId)
      state.loading = false
    }

    const editCash = async (cashId: string, cashData: any) => {
      state.loading = true
      await cashierRepository.editCash(cashId, cashData)
      state.loading = false
    }

    const getCashStatement = async (cashId: string) => {
      state.loading = true
      const data = await cashierRepository.getCashStatement(cashId)
      state.loading = false
      return data
    }
    ...
    const getCashStatementServices = async (cashId: string) => {
      state.loading = true
      const data: IGlobalResponse<IExtractService> = await cashierRepository.getCashStatementServices(cashId, state.extractServicesPagination.page, state.extractServicesPagination.page_size)
    ...
    const openCash = async () => {
      ...
      await cashierRepository.openCash(parkingId, employeeId)
      await getMyCashes()
    }

    const getCashesLogsManual = async (cashId: string, typeFilter: string) => {
      state.loading = true
      const data = await cashierRepository.getCashesLogsManual(cashId, state.cashLogsPagination.page, state.cashLogsPagination.page_size, typeFilter)
    ...
    const postCashLogManual = async (cashLogData: any, typeFilter: string) => {
      const data = await cashierRepository.postCashLogManual(cashLogData)
      await getCashesLogsManual(data.cashier, typeFilter)
    }

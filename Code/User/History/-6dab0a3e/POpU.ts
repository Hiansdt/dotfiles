...
export const useCashController = defineStore<"cash", ICashController>(
  ...
  () => {
    ...
    const getCashesByStatus = async (status: string) => {
      state.loading = true;
      const parking_id = authController.currentTenant?.id
    ...
    const getMyCashes = async () => {
      ...
      const parking_id = authController.currentTenant?.id
      const data = await cashRepository.getCashes(employee_id, parking_id, state.myCashesPagination.page, state.myCashesPagination.pageSize)
    ...
    const openCash = async () => {
      const parkingId = authController.currentTenant?.id
      const employeeId = authController.currentEmployeeId
    ...
    const getCashesLogsManual = async (cashId: string, typeFilter: string) => {
      state.loading = true
      const data = await cashRepository.getCashesLogsManual(cashId, state.cashLogsPagination.page, state.cashLogsPagination.pageSize, typeFilter)

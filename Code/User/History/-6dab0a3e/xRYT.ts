...
export const useCashController = defineStore<"cash", ICashController>(
  ...
  () => {
    ...
    const getCashesByStatus = async (status: string) => {
      state.loading = true;
      const companyId = authController.currentTenant?.id
    ...
    const getMyCashes = async () => {
      state.loading = true
      const companyId = authController.currentTenant?.id
      const data = await cashRepository.getCashes(companyId, state.myCashesPagination.page, state.myCashesPagination.pageSize)
    ...
    const openCash = async () => {
      ...
      await cashRepository.openCash(companyId)
      await getMyCashes()
    }

...
const getOrOpenCash = async () => {
  await cashController.getCashesByStatus("OPEN");
  console.log(cashController.cashes)
}
...
watch(() => paymentMethodController.paymentMethods,() => {
  paymentFields.value[0].items = paymentMethodController.paymentMethods.map((paymentMethod) => ({
...
<template>
  ...
  <GenericForm
    ...
    @update:is-dialog-visible="(v: boolean) => togglePayModel(v)"
  />
</template>

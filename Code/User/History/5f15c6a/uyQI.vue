...
const paymentMethods = computed(() => {
  return new Set(sessionPaymentController.sessionPayments.map((sessionPayment) => sessionPayment.payment_method))
})

const calculatePaymentMethodTotal = (payment_method: string) => {
  let sum = 0;
  sessionPaymentController.sessionPayments.forEach((sessionPayment) => {
    if (sessionPayment.payment_method == payment_method) {
      sum += Number(sessionPayment.price)
    }
  })
  return sum
}
...
onMounted(async () => {
  cash.value = await cashController.getCashById(id.value);
  await sessionPaymentController.getSessionPaymentsByCash(
    cash.value.id as string,
  );
});
</script>
    ...
    <VCol cols="12" class="pa-0">
      ...
      >
        <VWindowItem>
          <VCard class="pa-4">
            <VRow>
              <VCol v-for="sessionPayment in sessionPaymentController.sessionPayments">
                <VCard class="pa-2" variant="outlined">
                  <VCardTitle>
                    {{ sessionPayment.payment_method }}
                  </VCardTitle>
                  <VCardSubtitle>
                    {{
                      sessionPayment.price
                        ? `R$ ${formatBrl(sessionPayment.price)}`
                        : "Não informado"
                    }}
                  </VCardSubtitle>
                </VCard>

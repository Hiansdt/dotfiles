...
const debounceCashes = debounce(getCashes);
...
<template>
  <GenericTable
    key-word="Caixa"
    v-model:headers="cashController.myCashesHeaders"
    v-model:search="cashController.search"
    v-model:pagination="cashController.myCashesPagination"
    v-model:items="cashController.myCashes"
    @updateOptions="debounceCashes"
    @update:search="debounceCashes"
    @create-item="showCreateModal = true"
    no-items-text="Nenhum caixa encontrado"
  >
    <VDataTable
      v-if="cashController.myCashes.length"
      :headers="
        cashController.myCashesHeaders.filter((header) => header.isVisible)
      "
      :items="cashController.myCashes"
      :loading="cashController.loading"
      :items-per-page="cashController.myCashesPagination.pageSize"
      :page="cashController.myCashesPagination.page"
      :search="cashController.search"
      class="text-no-wrap"
    >
      <template v-slot:header.actions>
        <div
          class="mx-4"
          :class="
            cashController.myCashesHeaders.find((h) => h.value == 'actions')
              ?.sequence ==
      ...
      <template #item.opened_at="{ item }">
        {{
          item.opened_at
            ? useDate().format(item.opened_at, "keyboardDateTime24h")
            : "Não Informado"
        }}
      </template>
      ...
      <template #item.closed_at="{ item }">
        {{
          item.closed_at
            ? useDate().format(item.closed_at, "keyboardDateTime24h")
            : "Não Informado"
        }}
      </template>
      ...
      <template #item.actions="{ item }">
        <div
          class="d-flex gap-1 align-center w-10"
          :class="
            cashController.myCashesHeaders.find((h) => h.value == 'actions')
              ?.sequence ==
            cashController.myCashesHeaders.length - 1
  ...
  <VDialog v-model="showCreateModal" max-width="500">
    <VCard class="pa-4">
      <VCardTitle class="d-flex flex-column align-center pa-4">
        <VIcon size="80"> ri-error-warning-line </VIcon>
        <p class="ma-0">Realmente deseja criar um caixa?</p>
      </VCardTitle>

      <VCardActions>
        <VRow>
          <VCol cols="6">
            <VBtn block variant="outlined" color=""> Cancelar </VBtn>
          </VCol>
          <VCol cols="6">
